...
QDoubleSpinBox *createDoubleSpinBox(
    ...
    double step, double value, double decimals, int row, int col) {
...
void DupComponentTool::Apply() {
  ...
  for (DLFLVertexPtrArray::iterator it = all_vertices.begin();
  ...
  for (DLFLFacePtrSet::iterator it = old_faces.begin();
    ...
    do {
      ...
      int uid = u->getID();
      int vid = v->getID();
      if (new_edges.find(uid) != new_edges.end() && 
          new_edges[uid].find(vid) != new_edges[uid].end())
        new_edge = new_edges[uid][vid];
