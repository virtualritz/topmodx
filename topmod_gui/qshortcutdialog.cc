...
static QString keyToString(int k) {
  if (k == Qt::Key_Shift || k == Qt::Key_Control || k == Qt::Key_Meta || k
      == Qt::Key_Alt || k == Qt::Key_AltGr) return QString::null;
...
static QStringList modToString(Qt::KeyboardModifiers k) {
  ...
  if (k & Qt::ShiftModifier) l << QShortcut::tr("Shift");
  if (k & Qt::ControlModifier) l << QShortcut::tr("Ctrl");
  if (k & Qt::AltModifier) l << QShortcut::tr("Alt");
  if (k & Qt::MetaModifier) l << QShortcut::tr("Meta");
...
public:
  ...
  QString exec(const QString& s) {
    ...
    if (QDialog::exec() == QDialog::Accepted) return leKey->text();
    ...
    switch (e->type()) {
      case QEvent::KeyPress:
        ...
        if (key.count() || mods.count()) {
          ...
          if (key.count() && !lKeys.contains(key)) lKeys << key;

          foreach ( key, mods )if ( !lKeys.contains(key) )
          lKeys << key;
...
void QShortcutDialog::exec() {
  ...
  for (i = pManager->m_actions.begin(); i != pManager->m_actions.end(); i++) {
    ...
    if (cxt.isEmpty() || i->isEmpty()) continue;

    QString name = cxt.takeAt(cxt.count() - 1), shortcut = i->at(0)->shortcut();
...
void QShortcutDialog::on_twShortcuts_itemDoubleClicked(QTreeWidgetItem *i,
  ...
  ns = ShortcutGetter().exec(pManager->node(name, cxt.join("/")).attribute(
      "shortcut"));
