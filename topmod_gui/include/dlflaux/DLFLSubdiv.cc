...
namespace DLFL {
  ...
  bool dooSabinSubdivide(DLFLObjectPtr obj,bool check/*, QProgressDialog *progress*/) {		
    ...
    cout << "DooSabin:" << std::clock() / (double)CLOCKS_PER_SEC << endl;
    ...
    while ( fl_first != fl_last && num_faces < num_old_faces ) {
      ...
      // cout << "DLFLSubdiv::dooSabinSubdivide: edgeindex = " << edgeindex << endl;
  ...
  void subdivideFaces(DLFLObjectPtr obj, DLFLFacePtrList fplist, bool usequads) {
    ...
    if ( usequads ) {
      ...
      // insertEdge() will add new faces to the fplist, which may cause problem!
      // we insert edges after the looop
      DLFLFaceVertexPtrArray new_edge_starts, new_edge_ends;
      ...
      while ( fpfirst != fplast ) {
				...
				for (int i=0; i < corners.size(); ++i) {
					if ( corners[i]->getType() == FVTNew ) {
						...
						corners[i]->getVertexPtr()->resetType();
            // skip self loop
            if (cenfvp->getVertexPtr() == corners[i]->getVertexPtr()) continue;
            new_edge_starts.push_back(cenfvp);
            new_edge_ends.push_back(corners[i]);
						// insertEdge(obj,cenfvp,corners[i]);
					}
				}
      }
      //insert edges here.
      for (int i = 0; i < new_edge_starts.size(); ++i) {
        insertEdge(obj, new_edge_starts[i], new_edge_ends[i]);
      }
