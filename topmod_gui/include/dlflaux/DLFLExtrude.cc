...
namespace DLFL {
    ...
    DLFLFacePtrArray new_faces;
		...
		if ( head ) {
			...
			new_faces = obj->createFace(newverts,fptr->material());
  ...
  //Cubical extrusion calls this function.
  ...
  DLFLFacePtr extrudeFaceScherkCollins(DLFLObjectPtr obj, DLFLFacePtr fptr, double d, int num,
      ...
      double pinch_width/*=1.0*/) {
		DLFLFacePtr endface = fptr;
    int scherk_collins_twist = 0;
    for (int i = 0; i < num; ++i) {
      Vector3d dir = endface->computeNormal();
      normalize(dir);
		  DLFLFacePtrArray new_faces = duplicateFace(obj,endface,dir,d,rot,sf);
      DLFLFacePtr theface = new_faces[0];
      if (theface != NULL) {
        // Get the pointer to the second newly inserted face
        DLFLFacePtr nfp = new_faces[1];
         
        // The last face will be the one facing the old face
        // Find the first face-vertices in the 2 faces to be connected
        DLFLFaceVertexPtr fvp1, fvp2;
        fvp1 = endface->firstVertex();
        fvp2 = nfp->firstVertex();
        if (triangulate_new_faces) dualConnectFaces(obj,fvp1,fvp2);
        else {
          DLFLEdgePtrArray new_edges = connectFaces(obj,fvp1,fvp2);
          if (scherk_collins && endface->size() % 2 == 0)
            ScherkCollinsHandle(obj, new_edges, pinch, pinch_center,
                                pinch_width, scherk_collins_twist);
        }
      }
      endface = theface;
	...
	DLFLFacePtr extrudeFace(DLFLObjectPtr obj, DLFLFacePtr fptr, double d, const Vector3d& dir, double rot, double sf) {
		DLFLFacePtr endface = duplicateFace(obj,fptr,dir,d,rot,sf)[0];
		if ( endface != NULL ) {
			// Get the pointer to the second newly inserted face
			DLFLFacePtr nfp = obj->lastFace();
