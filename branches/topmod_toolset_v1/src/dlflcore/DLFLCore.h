...
namespace DLFL {
   ...
   */
  DLFLEdgePtr insertEdge(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
      DLFLFaceVertexPtr fvptr2, bool set_type = false,
      DLFLMaterialPtr matl = NULL);

  DLFLEdgePtr insertEdgeCoFacial(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
      DLFLFaceVertexPtr fvptr2, bool set_type = false);

  DLFLEdgePtr insertEdgeNonCoFacial(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
      DLFLFaceVertexPtr fvptr2, DLFLMaterialPtr matl = NULL);

  DLFLEdgePtr insertEdgeWithoutCheck(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
      DLFLFaceVertexPtr fvptr2, bool set_type = false,
      DLFLMaterialPtr matl = NULL);
  ...
  // because of the edge deletion will be removed from the object
	std::vector<int>  deleteEdgeID(
      DLFLObjectPtr obj, uint edgeId, bool cleanup = true);

  void deleteEdge(DLFLObjectPtr obj, uint edge_index, bool cleanup = true);

  DLFLFacePtrArray deleteEdge(DLFLObjectPtr obj, DLFLEdgePtr edgeptr,
      bool cleanup = true);
  ...
  DLFLVertexPtr collapseEdge(
      DLFLObjectPtr obj, DLFLEdgePtr edgeptr, bool cleanup = true);
  ...
  DLFLVertexPtr subdivideEdge(
     DLFLObjectPtr obj, DLFLEdgePtr edgeptr, bool set_type = false);

  DLFLVertexPtr subdivideEdge(DLFLObjectPtr obj, uint edge_index);

  // Subdivide into multiple edges
  vector<int> subdivideEdgeID(
      DLFLObjectPtr obj, int num_divs, uint edgeId, bool set_type = false);

  DLFLVertexPtrArray subdivideEdge(
      DLFLObjectPtr obj, int num_divs, DLFLEdgePtr edgeptr,
      bool set_type = false);
  ...
  void trisectEdge(DLFLObjectPtr obj, DLFLEdgePtr edgeptr, double num,
                   bool scale, bool set_type);
  
  // Subdivide all edges in the object
  void subdivideAllEdges(
      DLFLObjectPtr obj, int num_divs = 2, bool set_type = false);

  // Subdivide all edges in the object which are not self loops
  void subdivideNonLoopingEdges(
      DLFLObjectPtr obj, int num_divs = 2, bool set_type = false);
  ...
  bool writeObjectFile(
      DLFLObject *obj, char* filename = NULL, char *mtlfilename=NULL);
	...
	uint* createVertex(
      double x, double y, double z, DLFLObjectPtr &obj, bool set_type = false);
