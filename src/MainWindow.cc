...
MainWindow::MainWindow(char *filename) :
  object(), mode(NormalMode), undoList(), undoMtlList(), redoList(),
      redoMtlList(), undolimit(20), useUndo(true), mIsModified(false),
      mIsPrimitive(false), mWasPrimitive(false), mSpinBoxMode(None) {
  ...
  active = new GLWidget(500, 600, lit, QColor(255, 255, 255, 255), QColor(255,
      255, 255, 255), &object, fmt, this);
  ...
  tool_options_dock_ = new QDockWidget(tr("Tool Options"), this);
  // tool_options_dock_->setFeatures(QDockWidget::DockWidgetFloatable | QDockWidget::DockWidgetMovable /* | QDockWidget::DockWidgetClosable*/);
  tool_options_dock_->setAllowedAreas(Qt::NoDockWidgetArea);
  tool_options_stack_ = new QStackedWidget();
  tool_options_dock_->setWidget(tool_options_stack_);
  tool_options_dock_->setFloating(true);

  //make a new instance of QShortcutManager
  shortcut_manager_ = new QShortcutManager();
  action_model_ = new QStandardItemModel();
  action_list_widget_ = new QWidget;

  //startup dialog
  startup_dialog_dock_ = new QDockWidget(tr("Learning Movies"), this);

  createActions();

  //tools action group initialization
  //this widget allows the proper icon to be highlighted in the toolbar, and
  //only one icon to be highlighted at a time.
  tools_action_group_ = new QActionGroup(this);
  tools_action_group_->setExclusive(true);

  //instantiate toolbars
  basic_toolset_ = new BasicToolset(this, shortcut_manager_, action_list_widget_,
      tools_action_group_, tool_options_stack_);
  extrusion_toolset_ = new ExtrusionToolset(this, shortcut_manager_, action_list_widget_,
      tools_action_group_, tool_options_stack_);
  conical_toolset_ = new ConicalToolset(this, shortcut_manager_, action_list_widget_,
      tools_action_group_, tool_options_stack_);
  remeshing_toolset_ = new RemeshingToolset(this, shortcut_manager_, action_list_widget_,
      tools_action_group_, tool_options_stack_);
  high_genus_toolset_ = new HighGenusToolset(this, shortcut_manager_, action_list_widget_,
      tools_action_group_, tool_options_stack_);
  texturing_toolset_ = new TexturingToolset(this, shortcut_manager_, action_list_widget_,
      tools_action_group_, tool_options_stack_);
  experimental_toolset_ = new ExperimentalToolset(this, shortcut_manager_, action_list_widget_,
      tools_action_group_, tool_options_stack_);

#ifdef QCOMPLETER
  mCommandCompleter = new CommandCompleter(action_list_widget_, this);
#endif
  createToolBars();
  createMenus();
  ...
  mPreferencesDialog
      = new TopModPreferences(settings_, mStyleSheetEditor, shortcut_manager_, this);

  // //testing shortcut context
  // for (int i = 0; i < action_list_widget_->actions().count(); i++){
  // 	action_list_widget_->actions().at(i)->setShortcutContext(Qt::WidgetShortcut);
  // }

  //reposition floating windows:
  // tool_options_dock_->setGeometry(10 + this->x()/*this->active->width()-tool_options_dock_->width()*/,this->y()+150,tool_options_dock_->width(),tool_options_dock_->height());
  // tool_options_dock_->setFixedSize(tool_options_dock_->width(),tool_options_dock_->height());
...
void MainWindow::createActions() {
  ...
  //File Menu Actions
  mNewFileAct
      = new QAction(/*QIcon(":images/folder.png"),*/tr("&New File..."), this);
  ...
  action_list_widget_->addAction(mNewFileAct);
  ...
  action_list_widget_->addAction(mOpenAct);
  ...
  action_list_widget_->addAction(mSaveAct);

  mSaveAsAct = new QAction(QIcon(":images/document-save-as.png"), tr(
      "Save &As..."), this);
  ...
  action_list_widget_->addAction(mSaveAsAct);

  mSavePatchesAct = new QAction(QIcon(":images/saveas.png"), tr(
      "Save &Patch OBJ..."), this);
  ...
  action_list_widget_->addAction(mSavePatchesAct);

  mSaveLG3dAct = new QAction(QIcon(":images/saveas.png"), tr(
      "Export LiveGrahpics3D..."), this);
  sm->registerAction(mSaveLG3dAct, "File Menu", "CTRL+ALT+S");
  mSaveLG3dAct->setStatusTip(
      tr(
          "Export a LiveGraphics3D (*.m) file for embedding into the TopMod Wiki, Warning: you cannot import this file back into TopMod"));
  connect(mSaveLG3dAct, SIGNAL(triggered()), this, SLOT(saveFileLG3d()));
  action_list_widget_->addAction(mSaveLG3dAct);

  mSaveLG3dSelectedAct = new QAction(QIcon(":images/saveas.png"), tr(
      "Export LG3d (Sel. Faces)..."), this);
  sm->registerAction(mSaveLG3dSelectedAct, "File Menu", "");
  mSaveLG3dSelectedAct->setStatusTip(
      tr(
          "Export a LiveGraphics3D (*.m) of the current selected faces file for embedding into the TopMod Wiki, Warning: you cannot import this file back into TopMod"));
  connect(mSaveLG3dSelectedAct, SIGNAL(triggered()), this, SLOT(saveFileLG3dSelected()));
  action_list_widget_->addAction(mSaveLG3dSelectedAct);

  mExportSTLAct
      = new QAction(QIcon(":images/saveas.png"), tr("Export STL..."), this);
  sm->registerAction(mExportSTLAct, "File Menu", "CTRL+ALT+SHIFT+S");
  mExportSTLAct->setStatusTip(
      tr(
          "Export a stereolithography (*.stl) file for use with various rapid prototyping software and hardware"));
  connect(mExportSTLAct, SIGNAL(triggered()), this, SLOT(saveFileSTL()));
  action_list_widget_->addAction(mExportSTLAct);

  mScreenshotViewportAct = new QAction(QIcon(":images/camera-photo.png"), tr(
      "Save Viewport Screenshot..."), this);
  sm->registerAction(mScreenshotViewportAct, "File Menu", "0");
  mScreenshotViewportAct->setStatusTip(tr(
      "Save a screenshot of the contents of the OpenGL viewport (*.png)"));
  connect(mScreenshotViewportAct, SIGNAL(triggered()), this, SLOT(viewportScreenshot()));
  action_list_widget_->addAction(mScreenshotViewportAct);

  mScreenshotAppAct = new QAction(QIcon(":images/image-x-generic.png"), tr(
      "Save App Screenshot..."), this);
  sm->registerAction(mScreenshotAppAct, "File Menu", "9");
  mScreenshotAppAct->setStatusTip(tr(
      "Save a screenshot of the entire main application window (*.png)"));
  connect(mScreenshotAppAct, SIGNAL(triggered()), this, SLOT(appScreenshot()));
  action_list_widget_->addAction(mScreenshotAppAct);
  ...
  action_list_widget_->addAction(loadTextureAct);
  ...
  action_list_widget_->addAction(printInfoAct);
  ...
  action_list_widget_->addAction(printFaceListAct);
  ...
  action_list_widget_->addAction(printVertexListAct);
  ...
  action_list_widget_->addAction(printEdgeListAct);
  ...
  action_list_widget_->addAction(mPrintCVListAct);
  ...
  // action_list_widget_->addAction(mExitAct);
  ...
  // action_list_widget_->addAction(mQuickCommandAct);
#endif
  //Edit Menu Actions
  mDeleteSelectedAct = new QAction(/*QIcon(":images/edit-undo.png"),*/tr(
  ...
  action_list_widget_->addAction(mDeleteSelectedAct);
  ...
  action_list_widget_->addAction(mUndoAct);
  ...
  action_list_widget_->addAction(mPerspViewAct);
  ...
  // action_list_widget_->addAction(mZoomInAct);
  ...
  // // action_list_widget_->addAction(mTopViewAct);
  //
  ...
  // // action_list_widget_->addAction(mBottomViewAct);
  //
  ...
  // // action_list_widget_->addAction(mFrontViewAct);
  //
  ...
  // // action_list_widget_->addAction(mBackViewAct);
  //
  ...
  // // action_list_widget_->addAction(mLeftViewAct);
  //
  ...
  // // action_list_widget_->addAction(mRightViewAct);

  //Display Menu Actions
  mFullscreenAct = new QAction(QIcon(":images/view-fullscreen.png"), tr(
      "&Full Screen"), this);
  ...
  action_list_widget_->addAction(mFullscreenAct);
  ...
  action_list_widget_->addAction(showVerticesAct);
  ...
  action_list_widget_->addAction(mShowFaceIDsAct);
  ...
  action_list_widget_->addAction(mShowEdgeIDsAct);
  ...
  action_list_widget_->addAction(mShowVertexIDsAct);
  ...
  action_list_widget_->addAction(mShowSelectedIDsAct);
  ...
  action_list_widget_->addAction(showSilhouetteAct);
  ...
  action_list_widget_->addAction(showWireframeAct);
  ...
  action_list_widget_->addAction(showCoordinateAxesAct);
  ...
  action_list_widget_->addAction(objectOrientationAct);
  ...
  action_list_widget_->addAction(mShowNormalsAct);
  ...
  action_list_widget_->addAction(mShowFaceCentroidsAct);
  ...
  action_list_widget_->addAction(showGridAct);
  ...
  action_list_widget_->addAction(showHUDAct);
  ...
  action_list_widget_->addAction(mUseGPUAct);
#endif
  ...
  action_list_widget_->addAction(mAntialiasingAct);
  ...
  action_list_widget_->addAction(mShowScriptEditorAct);
#endif

  mShowToolOptionsAct = tool_options_dock_->toggleViewAction();
  mShowToolOptionsAct->setStatusTip(tr("Show the tool options window"));
  sm->registerAction(mShowToolOptionsAct, "Window Menu", "O");
  action_list_widget_->addAction(mShowToolOptionsAct);

  mShowStartupDialogAct = startup_dialog_dock_->toggleViewAction();
  mShowStartupDialogAct->setStatusTip(tr(
      "Show the startup screen with links to video tutorials"));
  sm->registerAction(mShowStartupDialogAct, "Window Menu", "");
  action_list_widget_->addAction(mShowStartupDialogAct);
...
//  action_list_widget_->addAction(mShowAnimatedHelpAct);
  ...
  action_list_widget_->addAction(planarModelingAct);
  ...
  action_list_widget_->addAction(polygonalModelingAct);
  polygonalModelingAct->setText(tr("&Polygonal modeling"));
  polygonalModelingAct->setStatusTip(tr("Switch to polygonal modeling mode"));
  polygonalModelingAct->setChecked(true);
  ...
  action_list_widget_->addAction(patchModelingAct);
  ...
  wireframeRendererAct->setStatusTip(tr(
      "Switch the current renderer to Wireframe"));
  connect(wireframeRendererAct, SIGNAL(triggered()), this, SLOT(useWireframeRenderer()));
  action_list_widget_->addAction(wireframeRendererAct);
  ...
  action_list_widget_->addAction(normalRendererAct);
  ...
  action_list_widget_->addAction(lightedRendererAct);
  ...
  texturedRendererAct->setStatusTip(tr(
      "Switch the current renderer to Textured"));
  connect(texturedRendererAct, SIGNAL(triggered()), this, SLOT(useTexturedRenderer()));
  action_list_widget_->addAction(texturedRendererAct);
  ...
  texturedLightedAct->setStatusTip(tr(
      "Switch the current renderer to Textured Lit"));
  connect(texturedLightedAct, SIGNAL(triggered()), this, SLOT(useTexturedLitRenderer()));
  action_list_widget_->addAction(texturedLightedAct);
  ...
  //action_list_widget_->addAction(patchRendererAct);
  ...
  colorableRendererAct->setStatusTip(tr(
      "Switch the current renderer to Colorable"));
  connect(colorableRendererAct, SIGNAL(triggered()), this, SLOT(useColorableRenderer()));
  action_list_widget_->addAction(colorableRendererAct);

  mModeActionGroup = new QActionGroup(this);
  mModeActionGroup->setExclusive(true);
  mModeActionGroup->addAction(planarModelingAct);
  mModeActionGroup->addAction(polygonalModelingAct);
  mModeActionGroup->addAction(patchModelingAct);
  ...
  action_list_widget_->addAction(pCubeAct);

  pOctahedronAct = new QAction(QIcon(":/images/prim_octahedron.png"), tr(
      "&Octahedron"), this);
  ...
  action_list_widget_->addAction(pOctahedronAct);

  pTetrahedronAct = new QAction(QIcon(":/images/prim_tetrahedron.png"), tr(
      "&Tetrahedron"), this);
  ...
  action_list_widget_->addAction(pTetrahedronAct);

  pDodecahedronAct = new QAction(QIcon(":/images/prim_dodecahedron.png"), tr(
      "&Dodecahedron"), this);
  ...
  action_list_widget_->addAction(pDodecahedronAct);

  pIcosahedronAct = new QAction(QIcon(":/images/prim_icosahedron.png"), tr(
      "&Icosahedron"), this);
  ...
  action_list_widget_->addAction(pIcosahedronAct);

  pSoccerBallAct = new QAction(QIcon(":/images/prim_soccerball.png"), tr(
      "&Soccer ball"), this);
  ...
  action_list_widget_->addAction(pSoccerBallAct);

  pGeodesicAct = new QAction(QIcon(":/images/prim_geodesic.png"), tr(
      "&Geodesic Dome"), this);
  ...
  action_list_widget_->addAction(pGeodesicAct);
  ...
  action_list_widget_->addAction(subdivideAllEdgesAct);
  ...
  action_list_widget_->addAction(planarizeAllFacesAct);
  ...
  action_list_widget_->addAction(makeObjectSphericalAct);
  ...
  action_list_widget_->addAction(mCleanup2gonsAct);

  mCleanupWingedVerticesAct
      = new QAction(tr("Remove valence-2 vertices"), this);
  ...
  action_list_widget_->addAction(mCleanupWingedVerticesAct);
  ...
  action_list_widget_->addAction(mSplitValence2VerticesAct);
  ...
  action_list_widget_->addAction(makeSierpinskiAct);
  ...
  action_list_widget_->addAction(computeLightingAct);

  computeNormalsAndLightingAct
      = new QAction(tr("Compute &Normals and Lighting"), this);
  ...
  action_list_widget_->addAction(computeNormalsAndLightingAct);

  assignTextureCoordinatesAct
      = new QAction(tr("Assign &Texture Coordinates"), this);
  ...
  action_list_widget_->addAction(assignTextureCoordinatesAct);
  ...
  action_list_widget_->addAction(mSelectionWindowAct);
  ...
  action_list_widget_->addAction(selectVertexAct);
  ...
  action_list_widget_->addAction(selectMultipleVerticesAct);
  ...
  action_list_widget_->addAction(mSelectMultipleAct);
  ...
  action_list_widget_->addAction(mEditVertexAct);
  ...
  action_list_widget_->addAction(selectFaceAct);
  ...
  action_list_widget_->addAction(selectFaceLoopAct);
  ...
  action_list_widget_->addAction(selectMultipleFacesAct);
  ...
  action_list_widget_->addAction(selectSimilarFacesAct);
  ...
  action_list_widget_->addAction(mSelectSimilarAct);
  ...
  action_list_widget_->addAction(selectFacesByAreaAct);
  ...
  action_list_widget_->addAction(selectFacesByColorAct);

  selectCheckerboardFacesAct
      = new QAction(tr("C&heckerboard Select Faces"), this);
  ...
  action_list_widget_->addAction(selectCheckerboardFacesAct);

  selectAllAct = new QAction(tr("Select &All"), this);
  sm->registerAction(selectAllAct, "Selection", "CTRL+A");
  connect(selectAllAct, SIGNAL( triggered() ), this, SLOT( selectAll() ) );
  action_list_widget_->addAction(selectAllAct);
  ...
  action_list_widget_->addAction(mSelectComponentAct);
  ...
  action_list_widget_->addAction(mGrowSelectionAct);
  ...
  action_list_widget_->addAction(mShrinkSelectionAct);
  ...
  action_list_widget_->addAction(selectInverseAct);
  ...
  action_list_widget_->addAction(selectEdgeAct);
  ...
  action_list_widget_->addAction(selectMultipleEdgesAct);
  ...
  action_list_widget_->addAction(mCollapseSelectedEdgesAct);
  ...
  action_list_widget_->addAction(selectEdgeLoopAct);
  ...
  action_list_widget_->addAction(selectEdgeRingAct);
  ...
  action_list_widget_->addAction(selectCornerAct);
  ...
  action_list_widget_->addAction(exitSelectionModeAct);
  ...
  action_list_widget_->addAction(clearSelectedModeAct);
  ...
  action_list_widget_->addAction(selectEdgesFromFacesAct);

  selectEdgesFromVerticesAct
      = new QAction(tr("Select Edges from Vertices"), this);
  ...
  action_list_widget_->addAction(selectEdgesFromVerticesAct);
  ...
  action_list_widget_->addAction(selectFacesFromEdgesAct);

  selectFacesFromVerticesAct
      = new QAction(tr("Select Faces from Vertices"), this);
  ...
  action_list_widget_->addAction(selectFacesFromVerticesAct);

  selectVerticesFromEdgesAct
      = new QAction(tr("Select Vertices from Edges"), this);
  ...
  action_list_widget_->addAction(selectVerticesFromEdgesAct);

  selectVerticesFromFacesAct
      = new QAction(tr("Select Vertices from Faces"), this);
  ...
  action_list_widget_->addAction(selectVerticesFromFacesAct);

  //selection masks
  mSelectVerticesMaskAct = new QAction(QIcon(
      ":images/selection-mask-vertices.png"), tr("Select &Vertices"), this);
  ...
  action_list_widget_->addAction(mSelectVerticesMaskAct);

  mSelectEdgesMaskAct
      = new QAction(QIcon(":images/selection-mask-edges.png"), tr(
          "Select &Edges"), this);
  ...
  action_list_widget_->addAction(mSelectEdgesMaskAct);

  mSelectFacesMaskAct
      = new QAction(QIcon(":images/selection-mask-faces.png"), tr(
          "Select &Faces"), this);
  ...
  action_list_widget_->addAction(mSelectFacesMaskAct);

  mSelectCornersMaskAct = new QAction(QIcon(
      ":images/selection-mask-corners.png"), tr("Select &Corner"), this);
  ...
  action_list_widget_->addAction(mSelectCornersMaskAct);
  ...
  //SETTINGS ACTIONS
  mPreferencesAct = new QAction(QIcon(":images/preferences-system.png"), tr(
      "&Preferences"), this);
  ...
  action_list_widget_->addAction(mPreferencesAct);
  ...
  action_list_widget_->addAction(englishAct);
  ...
  action_list_widget_->addAction(spanishAct);
  ...
  action_list_widget_->addAction(germanAct);
  ...
  action_list_widget_->addAction(frenchAct);
  ...
  action_list_widget_->addAction(italianAct);
  ...
  action_list_widget_->addAction(turkishAct);
  ...
  action_list_widget_->addAction(catalanAct);
  ...
  action_list_widget_->addAction(hindiAct);
  ...
  action_list_widget_->addAction(mPaintSelectedFacesAct);
  ...
  action_list_widget_->addAction(mClearMaterialsAct);

  mSubdivideSelectedFacesAct
      = new QAction(tr("Subdivide Selected Faces"), this);
  ...
  action_list_widget_->addAction(mSubdivideSelectedFacesAct);

  mSubdivideSelectedEdgesAct
      = new QAction(tr("Subdivide Selected Edges"), this);
  ...
  action_list_widget_->addAction(mSubdivideSelectedEdgesAct);
  ...
  action_list_widget_->addAction(mPerformRemeshingAct);

  mPerformExtrusionAct = new QAction(tr("Perform Extrusion"), this);
  mPerformExtrusionAct->setStatusTip(tr(
      "Perform the current extrusion operator on the selected faces"));
  ...
  action_list_widget_->addAction(mPerformExtrusionAct);
  ...
  mExtrudeMultipleAct->setStatusTip(
      tr(
          "Check this if you want to be able to select multiple faces and then hit CTRL+X to extrude the selected faces"));
  ...
  action_list_widget_->addAction(mExtrudeMultipleAct);

  //help menu actions
  mHelpAct = new QAction(QIcon(":images/applications-internet.png"), tr(
      "&Online User Manual"), this);
  ...
  action_list_widget_->addAction(mHelpAct);

  mVideoTutorialsAct
      = new QAction(QIcon(":images/applications-internet.png"), tr(
          "&TopMod Video Tutorials on blip.tv"), this);
  ...
  action_list_widget_->addAction(mVideoTutorialsAct);

  mTopModWebAct = new QAction(QIcon(":images/applications-internet.png"), tr(
      "&TopMod on the Web"), this);
  ...
  action_list_widget_->addAction(mTopModWebAct);

  mTopModResearchAct
      = new QAction(QIcon(":images/applications-internet.png"), tr(
          "&TopMod Research Papers"), this);
  ...
  action_list_widget_->addAction(mTopModResearchAct);
  ...
  action_list_widget_->addAction(mCheckForUpdatesAct);
  ...
  action_list_widget_->addAction(mAboutAct);
  ...
  action_list_widget_->addAction(mAboutQtAct);
  ...
  action_list_widget_->addAction(mHideToolBarsAct);
  ...
  action_list_widget_->addAction(mShowToolBarsAct);
...
void MainWindow::createMenus() {
  ...
  mModeMenu = new QMenu(tr("&Mode"));
  mModeMenu->setTearOffEnabled(true);
  mDisplayMenu->addMenu(mModeMenu);
  mModeMenu->addAction(planarModelingAct);
  mModeMenu->addAction(polygonalModelingAct);
  mModeMenu->addAction(patchModelingAct);
  ...
  // mRendererMenu->addAction(patchModelingAct);
  ...
  mToolsMenu->addMenu(basic_toolset_->menu_);
  mToolsMenu->addMenu(extrusion_toolset_->getMenu());
  mToolsMenu->addMenu(conical_toolset_->getMenu());
  mToolsMenu->addMenu(high_genus_toolset_->getMenu());
  mToolsMenu->addMenu(texturing_toolset_->getMenu());
  mToolsMenu->addMenu(experimental_toolset_->getMenu());
#ifdef QCOMPLETER
  ...
  mRemeshingMenu = remeshing_toolset_->getMenu();
  ...
  mWindowMenu->addAction(basic_toolset_->show_toolbar_action_);
...
void MainWindow::createToolBars() {
  ...
  //mBasicsMode->addActions(tools_action_group_, mToolsToolBar, tool_options_stack_);
  extrusion_toolset_->addActions(tools_action_group_, mExtrusionToolBar,
      tool_options_stack_);
  conical_toolset_->addActions(tools_action_group_, mConicalToolBar,
      tool_options_stack_);

  mRemeshingActionGroup = new QActionGroup(this);
  remeshing_toolset_->addActions(tools_action_group_, mRemeshingToolBar,
      tool_options_stack_);

  high_genus_toolset_->addActions(tools_action_group_, mHighgenusToolBar,
      tool_options_stack_);
  texturing_toolset_->addActions(tools_action_group_, mTexturingToolBar,
      tool_options_stack_);
  experimental_toolset_->addActions(tools_action_group_, mExperimentalToolBar,
      tool_options_stack_);
...
void MainWindow::createStartupDialog() {
  ...
  // startup_dialog_dock_->setFeatures(QDockWidget::DockWidgetFloatable | QDockWidget::DockWidgetMovable /* | QDockWidget::DockWidgetClosable*/);
  startup_dialog_dock_->setAllowedAreas(Qt::NoDockWidgetArea);
  startup_dialog_dock_->hide();

  startup_dialog_widget_ = new QWidget;
  startup_dialog_layout_ = new QGridLayout;

  //actions for the startup screen
  mTutorialNavigationAct = new QAction(QIcon(":/images/tutorial_01.png"), tr(
      "Navigation Basics"), this);
  ...
  startup_dialog_layout_->addWidget(mTutorialNavigationButton, 1, 0);

  mTutorialInterfaceAct = new QAction(QIcon(":/images/tutorial_02.png"), tr(
      "Interface Basics"), this);
  ...
  startup_dialog_layout_->addWidget(mTutorialInterfaceButton, 2, 0);

  mTutorialBasicAct = new QAction(QIcon(":/images/tutorial_03.png"), tr(
      "Basic Operations"), this);
  ...
  startup_dialog_layout_->addWidget(mTutorialBasicButton, 3, 0);

  mTutorialExtrusionAct = new QAction(QIcon(":/images/tutorial_04.png"), tr(
      "Extrusion Operations"), this);
  ...
  startup_dialog_layout_->addWidget(mTutorialExtrusionButton, 4, 0);

  mTutorialRemeshingAct = new QAction(QIcon(":/images/tutorial_05.png"), tr(
      "Remeshing Operations"), this);
  ...
  startup_dialog_layout_->addWidget(mTutorialRemeshingButton, 1, 1);

  mTutorialHighgenusAct = new QAction(QIcon(":/images/tutorial_06.png"), tr(
      "High genus Operations"), this);
  ...
  startup_dialog_layout_->addWidget(mTutorialHighgenusButton, 2, 1);

  mTutorialTexturingAct = new QAction(QIcon(":/images/tutorial_07.png"), tr(
      "Texturing Operations"), this);
  ...
  startup_dialog_layout_->addWidget(mTutorialTexturingButton, 3, 1);

  //show on startup? checkbox
  mShowStartupDialogAtStartupCheckBox = new QCheckBox(tr(
  ...
  startup_dialog_layout_->addWidget(mShowStartupDialogAtStartupCheckBox, 5, 0);

  //download quicktime link label
  quicktimeLabel = new QLabel(tr("Quicktime 7.2 or greater recommended."));
  downloadQuicktimeLabel
      = new QLabel(tr(
          "<h5>Quicktime 7.2 or greater recommended.<br /><a href=\"http://apple.com/quicktime/\">Download now.</a></h5>"));
  downloadQuicktimeLabel->setOpenExternalLinks(true);
  startup_dialog_layout_->addWidget(downloadQuicktimeLabel, 5, 1, Qt::AlignRight);

  startup_dialog_layout_->setColumnStretch(2, 1);
  startup_dialog_layout_->setRowStretch(6, 1);
  startup_dialog_widget_->setLayout(startup_dialog_layout_);
  startup_dialog_dock_->setWidget(startup_dialog_widget_);
  startup_dialog_dock_->setFloating(true);
  startup_dialog_dock_->move(this->width() / 2 + this->x()
      - startup_dialog_dock_->width() / 2, this->y() + height() / 2
      - startup_dialog_dock_->height() / 2);
  if (!mShowStartupDialogAtStartup) startup_dialog_dock_->hide();
}
...
void MainWindow::loadNavigationTutorial() {
  QDesktopServices::openUrl(
      QUrl::fromLocalFile("video/tutorial_navigation.mov"));
}
...
void MainWindow::setToolOptions(QWidget *options_widget) {
  tool_options_dock_->setWindowTitle(tr("Tool Options - ")
      + options_widget->windowTitle());
  tool_options_stack_->setCurrentWidget(options_widget);
  tool_options_stack_->updateGeometry();
  // show or hide the dockwidget options
  // if (options_widget->windowTitle() != "" && tool_options_dock_->isHidden())
  // tool_options_dock_->show();
  // else if (!tool_options_dock_->isHidden() && options_widget->windowTitle() == "")
  // tool_options_dock_->hide();
}
...
bool MainWindow::maybeSave() {
  if (this->isModified()) {
    int ret = QMessageBox::warning(this, tr("TopMod"), tr(
...
void MainWindow::about() {
  ...
  QString
      s =
          QString(
              "TopMod Topological Mesh Modeler v2.444 <br /><a href=\"http://www.topmod3d.org/\">TopMod Home Page</a><br /><a href=\"http://code.google.com/p/topmod/downloads/list\">Check for Updates.</a>");
...
void MainWindow::topModResearch() {
  QDesktopServices::openUrl(QUrl(
      "http://www.topmod3d.org/wiki/index.php?title=Research"));
}
  ...
  switch (mode) {
    case EditVertex: {
      if (GLWidget::numSelectedLocators() > 0) {
        ...
        gluProject(obj_world[0], obj_world[1], obj_world[2], modelMatrix,
            projMatrix, viewport, &obj_window[0], &obj_window[1],
            &obj_window[2]);

        // Set start and end window coordinates using depth coordinate found above
        ms_window[0] = drag_startx;
        ms_window[1] = drag_starty;
        ms_window[2] = obj_window[2];
        me_window[0] = drag_endx;
        me_window[1] = drag_endy;
        me_window[2] = obj_window[2];

        // Unproject start drag window coordinates to world coordinates
        gluUnProject(ms_window[0], ms_window[1], ms_window[2], modelMatrix,
            projMatrix, viewport, &ms_world[0], &ms_world[1], &ms_world[2]);

        // Unproject end drag window coordinates to world coordinates
        gluUnProject(me_window[0], me_window[1], me_window[2], modelMatrix,
            projMatrix, viewport, &me_world[0], &me_world[1], &me_world[2]);
...
void MainWindow::doSelection(int x, int y) {
  ...
  switch (mode) {
    ...
    case SelectEdgeLoop:
      ...
      septr = active->selectEdge(x, y);
      if (septr && QApplication::keyboardModifiers() == Qt::ControlModifier
          && active->isSelected(septr)) {
        deselect_edges = true;
        active->clearSelectedEdge(septr);
        num_sel_edges--;
    ...
    case EyeDropper://dave
      ...
      if (patch != active->getRenderer())
        ...
        // statusBar()->showMessage(QString("Trying to select face at position (%1, %2), got %3").arg(x).arg(y).arg((int)sfptr));
      }
    ...
    case SelectSimilar:
      ...
      switch (selectionmask) {
        case MaskFaces:
          ...
          //clear selection if shift isn't down
          if (QApplication::keyboardModifiers() != Qt::ShiftModifier) active->clearSelectedFaces();
          sfptr = active->selectFace(x, y);
          if (sfptr) {
            if (!active->isSelected(sfptr)) {
              // active->setSelectedFace(num_sel_faces,sfptr);
        ...
        case MaskEdges:
          // } else if (selectionmask == MainWindow::MaskEdges){
          //clear selection if shift isn't down
          if (QApplication::keyboardModifiers() != Qt::ShiftModifier) active->clearSelectedEdges();
          septr = active->selectEdge(x, y);
          if (septr) {
            if (!active->isSelected(septr)) {
              active->setSelectedEdge(septr);
        ...
        case MaskVertices:
          //clear selection if shift isn't down
          if (QApplication::keyboardModifiers() != Qt::ShiftModifier) active->clearSelectedVertices();
          svptr = active->selectVertex(x, y);
          if (svptr) {
            if (!active->isSelected(svptr)) {
              active->setSelectedVertex(svptr);
    ...
    case SelectFacesByArea:
      ...
      if (sfptr) {
        ...
        DLFLFacePtrArray sfptrarray;
        vector<DLFLFacePtr>::iterator it;
        DLFL::selectFacesByArea(&object, sfptr, sfptrarray,
            MainWindow::face_area_tolerance);
        for (it = sfptrarray.begin(); it != sfptrarray.end(); it++) {
          if (!active->isSelected(*it)) {
            active->setSelectedFace(*it);
            num_sel_faces++;
    ...
    case SelectFacesByColor:
      ...
      if (sfptr) {
        ...
        DLFLFacePtrArray sfptrarray;
        vector<DLFLFacePtr>::iterator it;
        DLFL::selectFacesByColor(&object, sfptr, sfptrarray,
            MainWindow::face_color_tolerance);
        for (it = sfptrarray.begin(); it != sfptrarray.end(); it++) {
          if (!active->isSelected(*it)) {
            active->setSelectedFace(*it);
            num_sel_faces++;
    ...
    case SelectCheckerboard:
      ...
      sfptr = active->selectFace(x, y);
      if (sfptr && active->isSelected(sfptr)
    ...
    case MultiSelectCorner:
      ...
      if (sfptr) {
        sfvptr = active->selectFaceVertex(sfptr, x, y);
        if (!active->isSelected(sfvptr)) active->setSelectedFaceVertex(sfvptr);
      }
      break;
...
void MainWindow::dropEvent(QDropEvent *event) {
  ...
  loadFile(
      QFileInfo(event->mimeData()->urls()[0].toLocalFile()).absoluteFilePath());
  event->acceptProposedAction();
}
...
void MainWindow::getRightClickMenu() {
  ...
  switch (mode) {
    ...
    break;
    SelectVertex: break;
    SelectEdge: mRightClickMenu->addAction(mSubdivideSelectedEdgesAct);
    break;
    SelectFace: mRightClickMenu->addAction(mSubdivideSelectedFacesAct);
    mRightClickMenu->addAction(mPaintSelectedFacesAct);
    break;
    SelectCorner: break;
    MultiSelectVertex: break;
    MultiSelectEdge: mRightClickMenu->addAction(mSubdivideSelectedEdgesAct);
    break;
    MultiSelectFace: mRightClickMenu->addAction(mSubdivideSelectedFacesAct);
    mRightClickMenu->addAction(mPaintSelectedFacesAct);
    break;
    MultiSelectCorner: break;
    InsertEdge: break;
    DeleteEdge: break;
    SubdivideEdge: break;
    CollapseEdge: mRightClickMenu->addAction(mSubdivideSelectedEdgesAct);
    mRightClickMenu->addAction(mCollapseSelectedEdgesAct);
    break;
    SpliceCorners: break;
    ConnectEdges: mRightClickMenu->addAction(mSubdivideSelectedEdgesAct);
    mRightClickMenu->addAction(mCollapseSelectedEdgesAct);
    break;
    ExtrudeFace: ExtrudeFaceDS: ExtrudeDualFace: ExtrudeFaceDodeca: ExtrudeFaceIcosa: ExtrudeMultipleFaces: StellateFace: DoubleStellateFace: ExtrudeFaceDome: mRightClickMenu->addAction(
        mPerformExtrusionAct);
    mRightClickMenu->addAction(mSubdivideSelectedFacesAct);
    mRightClickMenu->addAction(mPaintSelectedFacesAct);
    break;
    ConnectFaceVertices: ConnectFaces: BezierConnectFaces: HermiteConnectFaces: mRightClickMenu->addMenu(
        high_genus_toolset_->getMenu());
    break;
    ReorderFace: break;
    SubdivideFace: mRightClickMenu->addAction(mSubdivideSelectedFacesAct);
    break;
    CrustModeling: mRightClickMenu->addAction(createCrustThicknessAct);
    mRightClickMenu->addAction(createCrustScalingAct);
    mRightClickMenu->addSeparator();
    break;
    CutEdge: CutVertex: CutFace: mRightClickMenu->addAction(mPerformCuttingAct);
    mRightClickMenu->addSeparator();
    break;
    TruncateEdge: break;
    MarkEdge: break;
    MarkVertex: break;
    ConvexHullMode: break;
    EditVertex: break;
    SelectEdgeLoop: SelectEdgeRing: mRightClickMenu->addAction(
        mSubdivideSelectedEdgesAct);
    mRightClickMenu->addAction(mCollapseSelectedEdgesAct);
    ...
    SelectFaceLoop:
    // SelectSimilarFaces :
    SelectFacesByArea: SelectFacesByColor: SelectCheckerboard: mRightClickMenu->addAction(
        mSubdivideSelectedFacesAct);
    mRightClickMenu->addAction(mPaintSelectedFacesAct);
    break;
  ...
  // mRightClickMenu->addAction(mEditVertexAct);
  mRightClickMenu->addSeparator();
  mRightClickMenu->addMenu(basic_toolset_->menu_);
...
void MainWindow::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (mode != NormalMode && event->buttons() == Qt::LeftButton)
    ...
    doDrag(event->x(), this->size().height() - mStatusBar->height()
        - event->y());
  // else if ( event->buttons() == Qt::RightButton && QApplication::keyboardModifiers() == Qt::ShiftModifier){
  // 	// event->ignore();
  ...
  // }
    ...
    switch (mSpinBoxMode) {
      case One:
        if (mSpinBoxOne != 0) mSpinBoxOne->setValue(mSpinBoxOne->value() + d);
        break;
      case Two:
        if (mSpinBoxTwo != 0) mSpinBoxTwo->setValue(mSpinBoxTwo->value() + d);
        break;
      case Three:
        if (mSpinBoxThree != 0) mSpinBoxThree->setValue(mSpinBoxThree->value()
            + d);
        break;
      case Four:
        if (mSpinBoxFour != 0) mSpinBoxFour->setValue(mSpinBoxFour->value() + d);
        break;
      case Five:
        if (mSpinBoxFive != 0) mSpinBoxFive->setValue(mSpinBoxFive->value() + d);
        break;
      case Six:
        if (mSpinBoxSix != 0) mSpinBoxSix->setValue(mSpinBoxSix->value() + d);
        break;
      default:
        break;
    };
...
void MainWindow::setSpinBoxes(QDoubleSpinBox *one, QDoubleSpinBox *two,
  ...
  if (mode != NormalMode) {
    switch (mode) {
      ...
      case SelectEdgeLoop:
        if (active->numSelectedEdges() >= 1) {
          DLFLEdgePtr septr = active->getSelectedEdge(0);
          if (septr) getEdgeLoopSelection(septr);
        }
        // active->clearSelectedEdges();
        active->redraw();
        break;
      ...
      case PaintFace:
        if (active->numSelectedFaces() >= 1) {
          ...
          fp->setMaterial(object.addMaterial(RGBColor(
              paint_bucket_color.redF(), paint_bucket_color.greenF(),
              paint_bucket_color.blueF())));
          // if ( m ){
          //
          // }
          // else {
      ...
      case EyeDropper:
        if (active->numSelectedFaces() >= 1) {
          ...
          setPaintBucketColor(QColor(fp->material()->color.r,
              fp->material()->color.g, fp->material()->color.b));
          //set the color over in the experimental mode interface thing...
          experimental_toolset_->setPaintBucketColor(QColor(
              fp->material()->color.r, fp->material()->color.g,
              fp->material()->color.b));
          active->clearSelectedFaces();
          redraw();
        }
      ...
      case ConnectFaceVertices:
        if (active->numSelectedCorners() >= 2) {
          ...
          if (sfvptr1 && sfvptr2) {
            undoPush();
            setModified(true);
            DLFL::connectFaces(&object, sfvptr1, sfvptr2, num_segments,
                max_segments, holeHandle_pinching_factor,
                holeHandle_pinch_center, holeHandle_pinch_width);
            active->clearSelectedFaces();
            active->clearSelectedCorners();
            num_sel_faceverts = 0;
            num_sel_faces = 0;
        ...
        if (mSingleClickExtrude) {
          if (active->numSelectedFaces() >= 1) {
            ...
            if (sfptr) {
              undoPush();
              setModified(true);
              DLFL::extrudeFace(&object, sfptr, extrude_dist, num_extrusions,
        ...
        if (mSingleClickExtrude) {
          if (active->numSelectedFaces() >= 1) {
            ...
            if (sfptr) {
              undoPush();
              setModified(true);
              DLFL::extrudeDualFace(&object, sfptr, extrude_dist,
                  num_extrusions, extrude_rot, extrude_scale,
        ...
        if (mSingleClickExtrude) {
          if (active->numSelectedFaces() >= 1) {
            ...
            if (sfptr) {
              undoPush();
              setModified(true);
              DLFL::extrudeFaceDodeca(&object, sfptr, extrude_angle,
                  num_extrusions, extrude_length1, extrude_length2,
                  extrude_length3, hexagonalize_dodeca_extrude);
        ...
        if (mSingleClickExtrude) {
          if (active->numSelectedFaces() >= 1) {
            ...
            if (sfptr) {
              undoPush();
              setModified(true);
              DLFL::extrudeFaceIcosa(&object, sfptr, extrude_angle_icosa,
                  num_extrusions, extrude_length1_icosa, extrude_length2_icosa,
                  extrude_length3_icosa);
        ...
        if (mSingleClickExtrude) {
          if (active->numSelectedFaces() >= 1) {
            ...
            if (sfptr) {
              undoPush();
              setModified(true);
              DLFL::extrudeFaceDome(&object, sfptr, extrude_dist, extrude_rot,
                  extrude_scale);
              active->recomputePatches();
              active->recomputeNormals();
            }
      ...
      case BezierConnectFaces:
        if (active->numSelectedCorners() >= 2) {
          ...
          if (sfvptr1 && sfvptr2) {
            undoPush();
            setModified(true);
            DLFL::bezierConnectFaces(&object, sfvptr1, sfvptr2, num_segments,
                nwt1, nwt2);
            active->clearSelectedFaces();
            active->clearSelectedCorners();
            num_sel_faceverts = 0;
          ...
          if (sfvptr1 && sfvptr2) {
            ...
            if (symmetric_weights)
              DLFL::hermiteConnectFaces(&object, sfvptr1, sfvptr2,
                  num_segments, nwt1, nwt1, max_segments, num_extra_twists,
                  pinching_factor, pinch_center, bubble_factor,
                  triangulate_new_faces, scherk_collins, scherk_collins_pinch,
                  scherk_collins_pinch_center, scherk_collins_pinch_width,
                  scherk_collins_hole_twist, scherk_collins_hole_init_skip,
                  scherk_collins_hole_skip, scherk_collins_hole_seg_skip,
                  scherk_collins_hole_num_segs);
            else
              DLFL::hermiteConnectFaces(&object, sfvptr1, sfvptr2,
                  num_segments, nwt1, nwt2, max_segments, num_extra_twists,
                  pinching_factor, pinch_center, bubble_factor,
                  triangulate_new_faces, scherk_collins, scherk_collins_pinch,
                  scherk_collins_pinch_center, scherk_collins_pinch_width,
                  scherk_collins_hole_twist, scherk_collins_hole_init_skip,
      ...
      for (it = sfptrarr.begin(); it != sfptrarr.end(); it++) {
        switch (extrusionmode) {
          case DooSabinExtrude:
            DLFL::extrudeFaceDS(&object, *it, extrude_dist, num_extrusions,
                ds_ex_twist, extrude_scale);
            break;
          case CubicalExtrude:
            ...
            DLFL::extrudeFaceScherkCollins(&object, *it, extrude_dist,
                num_extrusions, extrude_rot, extrude_scale,
                triangulate_new_faces, scherk_collins, scherk_collins_pinch,
                scherk_collins_pinch_center, scherk_collins_pinch_width);
            break;
          ...
          case IcosahedralExtrude:
            ...
            DLFL::extrudeFaceIcosa(&object, *it, extrude_angle_icosa,
                num_extrusions, extrude_length1_icosa, extrude_length2_icosa,
                extrude_length3_icosa);
          ...
          case DodecahedralExtrude:
            DLFL::extrudeFaceDodeca(&object, *it, extrude_angle,
                num_extrusions, extrude_length1, extrude_length2,
                extrude_length3, hexagonalize_dodeca_extrude);
          ...
          case OctahedralExtrude:
            DLFL::extrudeDualFace(&object, *it, extrude_dist, num_extrusions,
                extrude_rot, extrude_scale, dual_mesh_edges_check);
            break;
          case StellateExtrude:
            DLFL::stellateFace(&object, *it, extrude_dist);
            break;
          case DoubleStellateExtrude:
            DLFL::doubleStellateFace(&object, *it, extrude_dist);
            break;
          case DomeExtrude:
            DLFL::extrudeFaceDome(&object, *it, domeExtrudeLength_factor,
                domeExtrudeRotation_factor, domeExtrudeScale_factor);
            break;
          default:
            DLFL::extrudeFace(&object, *it, extrude_dist, num_extrusions,
                extrude_rot, extrude_scale);
...
void MainWindow::setRenderer(DLFLRendererPtr rp) {
  ...
  if (rp == patch) {
    active->setPatchMode(true);
    active->recomputePatches();
    redraw();
  } else
    active->setPatchMode(false);
}
...
void MainWindow::setMode(Mode m) {
  ...
  switch (mode) {
    ...
    case SelectCorner:
      ...
      break;
    case MultiSelectVertex:
      s = tr("Multi-Select Vertex");
      break;
    case MultiSelectEdge:
      s = tr("Multi-Select Edge");
      break;
    case MultiSelectFace:
      s = tr("Multi-Select Face");
      break;
    case MultiSelectCorner:
      s = tr("Multi-Select Corner");
      break;
    case SelectCheckerboard:
      s = tr("Select Checkerboard");
      break;
    case InsertEdge:
      s = tr("Insert Edge");
      break;
    ...
    case ConnectEdges:
      ...
      break;
    case ExtrudeFace:
      s = tr("Cubical Extrusion");
      break;
    case ExtrudeFaceDS:
      s = tr("Doo Sabin Extrusion");
      break;
    case ExtrudeDualFace:
      s = tr("Dual Extrusion");
      break;
    case ExtrudeFaceDodeca:
      s = tr("Dodecahedral Extrusion");
      break;
    case ExtrudeFaceIcosa:
      s = tr("Icosahedral Extrusion");
      break;
    case ExtrudeMultipleFaces:
      s = tr("Extrude Multiple Faces");
      break;
    case StellateFace:
      s = tr("Stellate Face");
      break;
    case DoubleStellateFace:
      s = tr("Double Stellate Face");
      break;
    case ExtrudeFaceDome:
      s = tr("Dome Extrusion");
      break;
    case ConnectFaceVertices:
      s = tr("Connect Corners");
      break;
    case ConnectFaces:
      s = tr("Connect Faces");
      break;
    case BezierConnectFaces:
      s = tr("Bezier Connect Faces");
      break;
    case HermiteConnectFaces:
      s = tr("Hermite Connect Faces");
      break;
    case ReorderFace:
      s = tr("Reorder Face");
      break;
    ...
    case MarkVertex:
      ...
      break;
    case ConvexHullMode:
      s = tr("Convex Hull Mode");
      break;
    case EditVertex:
      s = tr("Edit Vertex");
      break;
    case SelectEdgeLoop:
      s = tr("Select Edge Loop");
      break;
    case SelectEdgeRing:
      s = tr("Select Edge Ring");
      break;
    case SelectFaceLoop:
      s = tr("Select Face Loop");
      break;
    case SelectSimilar:
      s = tr("Select Similar");
      break;
    case SelectFacesByArea:
      s = tr("Select Faces By Surf. Area");
      break;
    case SelectionWindow:
      s = tr("Selection Window");
      break;
    default:
      s = tr("-");
      break;
...
void MainWindow::readObject(const char * filename, const char *mtlfilename) {
  ...
  else if (strstr(filename, ".obj") || strstr(filename, ".OBJ")) object.readObject(
      file, mtlfile);
  file.close();
}
...
void MainWindow::readObjectQFile(QString filename) {
  ...
  if (filename.indexOf(".dlfl") == filename.length() - 4 || filename.indexOf(
      ".dlfl") == filename.length() - 4)
    object.readDLFL(filestring, mtlfile);
  else if (filename.indexOf(".OBJ") == filename.length() - 4
      || filename.indexOf(".obj") == filename.length() - 4) object.readObject(
      filestring, mtlfile);
  file.close();
...
void MainWindow::writeObject(const char * filename, const char* mtlfilename,
...
void MainWindow::openFile(void) {
  QString
      fileName =
          QFileDialog::getOpenFileName(
              ...
              mSaveDirectory,
              tr(
                  "All Supported Files (*.obj *.dlfl);;Wavefront OBJ Files (*.obj);;DLFL Files (*.dlfl);;All Files (*)"),
  ...
  if (!fileName.isEmpty()) {
    ...
    QString mtlfile = mSaveDirectory + "/" + QFileInfo(fileName).baseName()
        + ".mtl";
...
bool MainWindow::saveFile(bool with_normals, bool with_tex_coords) {
  if (curFile != "untitled") {
    ...
    if (!curFile.isEmpty()) {
      if (mIncrementalSave) {
        if (incremental_save_count >= mIncrementalSaveMax) {
          ...
          if (incremental_save_count < 10)
            ...
            t = QString("0%1").arg(incremental_save_count);
      ...
      mtlFileName.replace(QString(".dlfl"), QString(".mtl"),
          Qt::CaseInsensitive);
      ...
      if (mIncrementalSave) incremental_save_count++;
      setModified(false);
      ...
      if (!fileName.isEmpty()) {
        ...
        if (!fileName.indexOf(".obj") || !fileName.indexOf(".dlfl")
            || !fileName.indexOf(".OBJ") || !fileName.indexOf(".DLFL")) fileName.append(
            ".obj");
        setCurrentFile(fileName);
        ...
        mtlFileName.replace(QString(".obj"), QString(".mtl"),
            Qt::CaseInsensitive);
        mtlFileName.replace(QString(".dlfl"), QString(".mtl"),
            Qt::CaseInsensitive);
        QByteArray ba2 = mtlFileName.toLatin1();
...
bool MainWindow::saveFileBezierOBJ() {
  QString fileName = QFileDialog::getSaveFileName(this, tr(
      "Save Bezier Patch (OBJ)..."), mSaveDirectory + "/" + curFile, tr(
      "Wavefront OBJ Files (*.obj);;All Files (*)"), 0,
...
bool MainWindow::saveFileLG3d() {
  QString fileName = QFileDialog::getSaveFileName(this, tr(
      "Export to LiveGraphics3D  (M)..."), mSaveDirectory + "/" + curFile, tr(
      "Mathematica Graphics3D Files (*.m);;All Files (*)"), 0,
      QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
    QByteArray ba = fileName.toLatin1();
    const char *filename = ba.data();
...
bool MainWindow::saveFileLG3dSelected() {
  QString fileName = QFileDialog::getSaveFileName(this, tr(
      "Export Selected Faces to LiveGraphics3D  (M)..."), mSaveDirectory + "/"
      + curFile, tr("Mathematica Graphics3D Files (*.m);;All Files (*)"), 0,
      QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
    QByteArray ba = fileName.toLatin1();
    const char *filename = ba.data();
...
bool MainWindow::saveFileSTL() {
  ...
  QString fileName = QFileDialog::getSaveFileName(this, tr("Export STL..."),
      mSaveDirectory + "/" + curFile, tr("STL Files (*.stl);;All Files (*)"),
      0, QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
    QByteArray ba = fileName.toLatin1();
    const char *filename = ba.data();
...
bool MainWindow::viewportScreenshot() {
  ...
  QString fileName = QFileDialog::getSaveFileName(this, tr(
      "Save Viewport Screenshot As"), initialPath, tr(
      "%1 Files (*.%2);;All Files (*)") .arg(format.toUpper()) .arg(format), 0,
      QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
...
bool MainWindow::appScreenshot() {
  ...
  QString fileName = QFileDialog::getSaveFileName(this, tr(
      "Save App Screenshot As"), initialPath, tr(
      "%1 Files (*.%2);;All Files (*)") .arg(format.toUpper()) .arg(format), 0,
      QFileDialog::DontUseSheet);
  if (!fileName.isEmpty()) {
...
//recurse through selected edge to get a list of
void MainWindow::getFaceLoopSelection(DLFLEdgePtr eptr, bool start,
                                      DLFLFacePtr face_loop_marker,
                                      bool select_face_loop) {
  if ((eptr == face_loop_start_edge) && !start) return;
...
//recurse through selected edge to get a list of
void MainWindow::getEdgeRingSelection(DLFLEdgePtr eptr, bool start,
                                      DLFLFacePtr face_loop_marker,
                                      bool select_face_loop) {
  if ((eptr == edge_ring_start_edge) && !start) return;
  ...
  if (fptr1 && fptr1->numFaceVertexes() == 4 && !(fptr1 == face_loop_marker)) {
    ...
    for (it = edges.begin(); it != edges.end(); it++) {
      if (*it == eptr) {
        getEdgeRingSelection(edges[(idx + 2) % 4], false, fptr1,
            select_face_loop);
      }
      idx++;
    }//end for loop
  ...
  if (fptr2 && fptr2->numFaceVertexes() == 4 && !(fptr2 == face_loop_marker)) {
    ...
    for (it = edges.begin(); it != edges.end(); it++) {
      if (*it == eptr) {
        getEdgeRingSelection(edges[(idx + 2) % 4], false, fptr2,
            select_face_loop);
        // return;
      }
      idx++;
...
void MainWindow::setPolygonalMode() {
  ...
  if (active->isInPatchMode()) useLightedRenderer();
}

void MainWindow::setPatchMode() {
  cout << "Switching to patch mode." << endl;
  basic_toolset_->toolbar_->show();
  mHighgenusToolBar->show();
  usePatchRenderer();
  lightedRendererAct->setEnabled(false);
  wireframeRendererAct->setEnabled(false);
  texturedLightedAct->setEnabled(false);
  colorableRendererAct->setEnabled(false);
  texturedRendererAct->setEnabled(false);
  normalRendererAct->setEnabled(false);
}
...
void MainWindow::retranslateUi() {
  ...
  basic_toolset_->retranslateUi();
  extrusion_toolset_->retranslateUi();
  remeshing_toolset_->retranslateUi();
  texturing_toolset_->retranslateUi();
  high_genus_toolset_->retranslateUi();
  conical_toolset_->retranslateUi();
  experimental_toolset_->retranslateUi();

  startup_dialog_dock_->setWindowTitle(tr("Learning Movies"));
  tool_options_dock_->setWindowTitle(tool_options_dock_->windowTitle());
...
#endif
  //Edit Menu Actions
  mDeleteSelectedAct->setText(/*QIcon(":images/edit-undo.png"),*/tr(
      "&Delete Selected"));
  mDeleteSelectedAct->setStatusTip(tr("Delete Selected"));
  ...
  mSelectComponentAct->setText(tr("Select Connected Component"));
  ...
  mShowStartupDialogAtStartupCheckBox->setText(
      tr("Show this dialog at startup"));
  quicktimeLabel->setText(tr("Quicktime 7.2 or greater recommended."));
  downloadQuicktimeLabel->setText(
      tr(
          "<h5>Quicktime 7.2 or greater recommended.<br /><a href=\"http://apple.com/quicktime/\">Download now.</a></h5>"));
