...
DLFLEdgePtr insertEdgeCoFacial(DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
                               DLFLFaceVertexPtr fvptr2, bool set_type) {
...
DLFLEdgePtr insertEdgeNonCoFacial(
    DLFLObjectPtr obj, DLFLFaceVertexPtr fvptr1,
    DLFLFaceVertexPtr fvptr2, DLFLMaterialPtr matl) {
  ...
  // Go through all vertices in fp2 and add them to the new face and delete them
  // from the old face.
  fvp = fp2->front();
...
DLFLEdgePtr insertEdgeWithoutCheck(
...
DLFLFacePtrArray deleteEdge(DLFLObjectPtr obj, DLFLEdgePtr edgeptr, bool cleanup) {
  if (edgeptr == NULL) return DLFLFacePtrArray();
  if (find(obj->beginEdge(), obj->endEdge(), edgeptr) == obj->endEdge())
     return DLFLFacePtrArray();
  ...
  if (f1 != f2) {
    ...
    // Start at vertex after fvpV1, remove from face and add to new face till
    // we reach fvpV2. Don 't add fvpV2 unless fvpV2 is right after fvpV1, in
    // which case we will have a point sphere
    fvpTemp = fvpV1->next();
    if (fvpTemp == fvpV2) {
      // Add fvpV2.nfp will be a point sphere.Add a copy since fvpV2 will be
      // deleted later on. Since nfp will be a point sphere adding a copy will
      // not affect any other pointers.
    ...
    //Remove fvpV2 from f1 and free the pointer.Remove fvpV2 from face - vertex
    //- list of its vertex also.
    f1->deleteVertexPtr(fvpV2);
    fvpV2->deleteSelfFromVertex();
    delete fvpV2;
    
    // Now check if fvpV1 is the only vertex in f1.If so f1 becomes a point
    // sphere. Don 't delete fvpV1 from f1. Otherwise delete fvpV1 from f1
...
DLFLVertexPtrArray subdivideEdge(
    DLFLObjectPtr obj, int num_divs, DLFLEdgePtr edgeptr, bool set_type) {
...
void trisectEdge(DLFLObjectPtr obj, DLFLEdgePtr edgeptr, double num, bool scale,
                 bool set_type) {
...
void cleanupWingedVertices(DLFLObjectPtr obj) {
  ...
  DLFLEdgePtrArray edges_to_be_removed;
  while (vfirst != vlast) {
    vp = (*vfirst); ++vfirst;
    // If vp is a valence 2 vertex, it will be removed, so increment the
    // iterator now itself.
    if (vp->valence() == 2) {
      ...
      if (pfvp == nfvp) {
        ...
        // We do not add loops
        if (pfvp->getVertexPtr() != nfvp->getVertexPtr())
          insertEdgeCoFacial(obj,pfvp,nfvp);

        // Now delete the 2 edges incident on the valence-2 vertex with cleanup
        edges_to_be_removed.push_back(eparray[0]);
        edges_to_be_removed.push_back(eparray[1]);
      }
    }
  }
  for (DLFLEdgePtrArray::iterator it = edges_to_be_removed.begin();
       it != edges_to_be_removed.end(); ++it) {
    deleteEdge(obj, *it, true);
  }
  cout << "Exiting cleanupWingedVertices()..." << endl;
  return;
  // Self loop formed faces are removed as point sphere.
  DLFLFacePtrArray faces(obj->beginFace(), obj->endFace());
  for (DLFLFacePtrArray::iterator it = faces.begin(); it != faces.end();
      ++it) {
    // if it is a point sphere, remove it.
    DLFLFacePtr this_face = *it;

    if (this_face->size() == 1) {
      DLFLFaceVertexPtr fvp = this_face->firstVertex();
      DLFLVertexPtr vp = fvp->vertex;
      obj->removeVertex(vp);
      delete vp;

      this_face->destroy();
      obj->removeFace(this_face);
      delete this_face;
    }
  }
}
